//
//  APITestKeyLog.swift
//  iosda-c6-frontend
//
//  Created by Richie Reuben Hermanto on 14/09/25.
//

import SwiftUI

struct APITestKeyLog: View {
    @State private var apiStatus: String = "Checking API..."
    private let service = KeyLogService() // pastikan kamu punya service yg ada getAllKeyLog

    var body: some View {
        VStack(spacing: 20) {
            Text(apiStatus)
                .font(.title2)
                .multilineTextAlignment(.center)
        }
        .padding()
        .onAppear {
            Task {
                await testFetchKeyLogs()
            }
        }
    }
    
    // MARK: - Test fetch key logs by unitId
    private func testFetchKeyLogs() async {
        // Pakai token asli dari backend
        NetworkManager.shared.bearerToken = """
        eyJhbGciOiJFZERTQSJ9..bkbLRPCxSB8GbHolitnD6fEqBkBkJeIqBRP1OMTi1op17yFGZNpfLm7cH-fMd3AgDnfV9p6MKFePYbG4w7ydDQ
        """
        
        let unitId = "474964f5-1930-42c7-a6c6-e2ace5b18bcc" 
        
        do {
            let keyLogs = try await service.getAllKeyLog(unitId: unitId)
            if keyLogs.isEmpty {
                apiStatus = "⚠️ No key logs found for unit \(unitId)"
            } else {
                // contoh tampilkan log pertama
                if let first = keyLogs.first {
                    apiStatus = """
                    ✅ KeyLogs fetched: \(keyLogs.count)
                    First Log ID: \(first.id)
                    """
                }
            }
        } catch {
            apiStatus = "❌ Error: \(error.localizedDescription)"
        }
    }
}

#Preview {
    APITestKeyLog()
}
